...
/**
 * Emits the start of a while loop in MIPS assembly.
 * @param n Unique identifier for the loop.
 */
void emitMIPSWhile(int n) {
    mainMIPS = fopen("MIPScode.asm", "a");
    inFunc = 1;  // Set context to within a function
    inLoop = 1;  // Set context to within a loop
    fclose(mainMIPS);
}

/**
 * Emits the end of a while loop in MIPS assembly with a condition.
 * @param var1 The first variable in the condition.
 * @param var2 The second variable in the condition.
...
void emitMIPSEndWhile(char var1[10], char var2[10], char type1[10], char type2[10], int n) {
    ...
    // Emit the condition string and loop back to the top
    fprintf(mainMIPS, "%s\n", condString);
    fprintf(mainMIPS, ".end while%d\n", n);

    inFunc = 0; // Reset function context
    inLoop = 0; // Reset loop context
